...
func RegisterHandlers(r *echo.Echo, service endpoints.Service, logger log.Logger, prefix string) {
	...
	rg.GET("/contractVehicles", res.query)
	rg.POST("/contractVehicles", res.create)
	rg.Match([]string{"PUT", "PATCH"}, "/contractVehicles/:id", res.update)
	rg.DELETE("/contractVehicles", res.delete)
}
