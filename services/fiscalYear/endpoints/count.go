...
func (s *service) Count(ctx context.Context, params FiscalYearQueryRequestParams) (int64, response.ErrorResponse) {
	var count int64

	where := makeFilters(params)

	tx := s.db.WithContext(ctx).Model(&models.FiscalYear{})
