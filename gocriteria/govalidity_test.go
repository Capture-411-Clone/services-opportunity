...
func TestValidateBody(t *testing.T) {
	t.Run("BodyIsNil", func(t *testing.T) {
		...
		schema := Schema{
			...
			"name":         New("name").LowerCase().In([]string{"dan", "christy"}).Required(),
	...
	t.Run("CustomValidatorError", func(t *testing.T) {
		r := &http.Request{
			...
			Body: io.NopCloser(strings.NewReader("{\"email\":\"sgh370@yahoo.com\",\"name\":\"dan\",\"lastName\":\"ghanbari\",\"age\":\"50\",\"url\":\"https://google.com\",\"json\":\"{\\\"key\\\":\\\"value\\\"}\",\"ip\":\"127.0.0.1\",\"filter[page]\":\"12\"}")),
		}
		schema := Schema{
			"email":    New("email").Email().MinLength(5).Required(),
			"name":     New("name").LowerCase().MinMaxLength(3, 20).In([]string{"dan", "christy"}).Required(),
			"lastName": New("lastname").LowerCase().MaxLength(25).Required(),
			"age":      New("age").Number().Default("20").Required(),
			"url":      New("url").Url().Required(),
			"json":     New("json").Json(),
	...
	t.Run("Default", func(t *testing.T) {
		r := &http.Request{
			...
			Body: io.NopCloser(strings.NewReader("{\"email\":\"sgh370@yahoo.com\",\"name\":\"dan\",\"lastName\":\"ghanbari\",\"url\":\"https://google.com\",\"json\":\"{\\\"key\\\":\\\"value\\\"}\",\"ip\":\"127.0.0.1\",\"filter[page]\":\"12\"}")),
		}
		schema := Schema{
			"email":    New("email").Email().MinLength(5).Required(),
			"name":     New("name").LowerCase().MinMaxLength(3, 20).In([]string{"dan", "christy"}).Required(),
...
func TestValidateQueries(t *testing.T) {
	...
	t.Run("Check Nested", func(t *testing.T) {
		r := httptest.NewRequest(http.MethodGet, `/?email=sgh370@yahoo.com&filter={"phone":{"op":"equal","value":1},"email":{"op":"equal","value":"sgh370@yahoo.com"},"name":{"op":"equal","value":"dan"},"lastName":{"op":"equal","value":"ghanbari"},"userName":{"op":"equal","value":"sgh370"},"nationalCode":{"op":"equal","value":"0720464201"},"birthdate":{"op":"equal","value":"a"},"avatarUrl":{"op":"equal","value":"a"},"suspended_at":{"op":"equal","value":"a"},"created_at":{"op":"equal","value":"a"}}`, errReader(0))
